...
from PyQt4.QtCore import Signal, QSize, Qt, QThread
...
QLabel, QLinearGradient, QLineEdit, QListWidget, QPalette, QPixmap, QPushButton, QStackedLayout, \
QStackedWidget, QTabWidget, QTableWidget, QTableWidgetItem, QTextEdit, QWidget
...
class Oberflaeche_Betrieb(QTabWidget):
    ...
    def __init__(self, parent = None):
        ...
        'Tab4 - Wartung'
        self.button_fernsteuerung.clicked.connect(self.fernsteuerung_zulassen)
...
#        self.wechseln.emit(2) ###
        ...
        'Motorsteuerung'
        self.motorsteuerung('programmAus')
    ...
    '''Methode fernsteuerung_anfragen - Aktualisiert das Textfeld in 
    Tab4 - Wartung - mit dem Namen anfragender Apps.'''
    def fernsteuerung_anfragen(self, app_anfrage):
        
        angefragte_apps = []
        
        'anfragende Apps in Liste eintragen'
        ...
        'Bedienelement aktivieren'
        self.button_fernsteuerung.setEnabled(True)
    
    '''Methode fernsteuerung_zulassen - Die Methode wird nach Klick auf
    den Button - Fernsteuerung zulassen - ausgeführt.'''
    def fernsteuerung_zulassen(self):
        ...
        if self.fernsteuerung == False:
            ...
            if self.textfeld2.currentItem():
                ...
                'ausgewählten Verbindungspartner an Server anmelden'
                self.auswahl_app = self.textfeld2.currentItem().text()
                self.server.verbindung_anmelden(self.auswahl_app)
                
                'Textfelder aktualisieren'
                self.button_fernsteuerung.setText('Verbindung trennen')
                self.textfeld3.setText('Fernsteuerung aktiv. Verbunden mit: '
                + self.auswahl_app)
            ...
            'Verbindungspartner beim Server abmelden'            
            self.server.verbindung_abmelden(True)
            
            'Textfelder aktualisieren'
            self.button_fernsteuerung.setText('Fernsteuerung zulassen')
...
#        self.button_wartung.setEnabled(False)
    ...
    def grafik_tab1_laden(self):
        ...
        self.button_fortsetzen = QPushButton('Fortsetzen')
    ...
    def grafik_tab4_laden(self):
        ...
        'Buttons instanziieren'
        self.button_fernsteuerung = QPushButton('Fernsteuerung zulassen')
        self.button_fernsteuerung.setFixedHeight(self.widget_hoehe2)
        self.button_fernsteuerung.setDisabled
        ...
        spalte2.addWidget(self.button_fernsteuerung)
        spalte2.addWidget(self.button_wartungSchliessen)
    ...
    def motorsteuerung(self, speichername):
...
#    und lagert das Objekt in einen eigenen Thread aus.'''
#    def motorsteuerung_fern(self, p1, p2, p3, p4, p5, p6, t):
#        
#        'QThread-Objekt instanziieren'
#        thread_motor = QThread()
#        
...
#        self.motor.steuerung_fern(p1, p2, p3, p4, p5, p6)
#        
#        'Motor-Objekt an den Thread übergeben'
#        self.motor.moveToThread(thread_motor)
#        
#        'Signale und Slots verbinden'
#        thread_motor.started.connect(self.motor.fernsteuerung)
#        self.motor.finished.connect(thread_motor.quit)
#        self.motor.finished.connect(self.motor.deleteLater) 
#        thread_motor.finished.connect(thread_motor.deleteLater)
#        
...
#        thread_motor.exec()
    ...
    '''Methode fernsteuerung - Die Methode erzeugt ein Motor-Objekt,
    übergibt den Fernsteuerungsvektor und die Synchronisationszeit und lagert 
    das Objekt in einen eigenen Thread aus. Im Unterschied zur normalen
    Fernsteuerung erlaubt diese Funktion die Übergabe von 6 Motorwinkeln'''
    def motorsteuerung_fern_absolut(self):
        ...
        'Signale und Slots verbinden'
        self.server.fernsteuerung.connect(self.motor.steuerung_fern_abs_vektor)
        self.server.fernsteuerung_aus.connect(self.motor.steuerung_fern_aus)  
...
#        self.server.antwort_lage(lage) ###
    ...
    def schnittstelle_oeffnen(self):
        ...
        self.server.fernsteuerung_ein.connect(self.motorsteuerung_fern_absolut)
        self.server.server_anfrage.connect(self.fernsteuerung_anfragen)
        self.server.server_daten.connect(self.server_protokoll_update)
...
#        self.server.antwort_winkel(winkel) ###
    ...
    '''Methode server_protokoll_update - Aktualisiert das Textfeld in 
    Tab4 - Wartung - mit dem Sendernamen und der codierten Nachricht.'''
    ...
    def start(self):
        ...
        self.motorsteuerung(speichername)
