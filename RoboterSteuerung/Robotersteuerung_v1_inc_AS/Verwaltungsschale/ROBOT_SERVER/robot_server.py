...
def control():
    with open(os.getcwd()+'/data/data.json') as data_file:
        ...
        with io.open('data/data.json', 'w', encoding='utf8') as outfile:
            ...
            REQUEST = robotserver.create_message(TO = 'ROBOTER_GUI', CORE = a)
...
def testfunction():
    REQUEST = robotserver.create_message(TO = 'ROBOTER_GUI', CORE = {"request":"current position"})
    robotserver.send(REQUEST)
    MESSAGE = robotserver.receive()
...
def control_start_command():
    print("TEST0")
    REQUEST = robotserver.create_message(TO = 'ROBOTER_GUI', CORE = '20')
    print("TEST1 "+str(REQUEST))
    robotserver.send(REQUEST)
    MESSAGE = robotserver.receive()
