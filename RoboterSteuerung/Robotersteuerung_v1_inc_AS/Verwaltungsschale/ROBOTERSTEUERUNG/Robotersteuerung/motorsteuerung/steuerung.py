...
class Motor(QObject):
    ...
    def __init__(self, port, parent = None):
        ...
        self.stable_Ver = [000, 110, 120, 130]
...
#        self.exiting = True
    ...
    def steuerung_fern(self):
        ...
        self.b_end_n = asin((self.z_end-27.25)/sqrt(self.z_end*self.z_end+self.y_end*self.y_end+self.x_end*self.x_end))-pi/2
        ...
        self.Spd_m = 4
        self.zwanz = 20 / 180 * pi
        self.hund = 160 / 180 * pi
    ...
    def steuerung_fern_vektor(self, x_ver, y_ver, z_ver, rot_ver, b_ver, greif_ver, ):
        ...
        try:
            ...
            '''Testweise Berechnung von beta'''
#            if self.z5in0[2] >= 0:
#                self.b_urs = sin(self.z5in0[2]*3.1415/2)*3.1415/4
#            if self.z5in0[2] < 0:
#                self.b_urs = sin(-self.z5in0[2]*3.1415/2)*3.1415/4 + 3.1415/4
                
            self.b_urs = self.b_end_n
#           self.b_urs = asin((self.z_urs-27.25)/sqrt(self.z_urs*self.z_urs+self.y_urs*self.y_urs+self.x_urs*self.x_urs))-pi/2
#            self.b_urs = asin(self.z5in0[2]/sqrt(self.x_urs*self.x_urs+self.y_urs*self.y_urs+self.z_urs*self.z_urs))-pi/2
        ...
        self.b_end_n = self.b_urs + b_ver * self.amplifier / 4 / 180 * 3.1415
        
        
        
        self.greif_end = self.greif_urs + greif_ver * self.amplifier / 4 * 1
        ...
        '''Nachträglich die Rotation des Greifers einfügen'''
        self.theta5_n = self.theta5_n + rot_ver
        

        if self.theta5_n < self.zwanz:
            self.theta5_n = self.zwanz
        if self.theta5_n > self.hund:
            self.theta5_n = self.hund
#        print("Theta 5 ist akutell: ", self.theta5_n)
